...
**Project Setup is complete** (see [docs/features/001_Project_Setup.md](docs/features/001_Project_Setup.md)): the monorepo scaffolding, backend skeleton, frontend skeleton, Docker, CI, and dev tooling all exist and are verified working. **Phase 0 is now fully closed** (001 through 009 all done — see [ROADMAP.md](ROADMAP.md)'s Phase 0 table). **Phase 1 (`010_Plugin_Framework`) is also done**: `backend/app/core/plugins.py` provides plugin discovery/registration, but no actual plugin exists yet. **Phase 2 has started with `011_Wake_Word`, also done**: `backend/app/core/voice/` integrates OpenWakeWord as an opt-in capability (`pip install '.[voice]'`) — the backend always boots safely without it, degrading to a documented push-to-talk fallback (`POST /api/v1/wake-word/trigger`); `wake_word_events` is the handoff contract `012_STT` (not started) will subscribe to. **`007_Dashboard` is done too**: the home route (`/`) is a real widget-hosting screen (greeting, clock, the existing health check, placeholder tiles for unbuilt features) — see `frontend-dashboard/src/components/dashboard/` and `frontend-dashboard/src/routes/DashboardPage.tsx`. **`008_Settings` is done as well**: `app_name`/`default_theme` are now DB-backed and writable at runtime (`GET`/`PATCH /api/v1/settings`, a Settings page in the dashboard) — this is the app's first real persistence, `backend/app/db/models.py`'s `SettingModel` plus `backend/alembic/`, introduced ahead of the originally-planned Phase 4 (see `BACKLOG.md`). **`009_Assistant_Onboarding` closes out Phase 0**: a first-run wizard (`frontend-dashboard/src/routes/OnboardingPage.tsx`, route `/onboarding`) walks a new user through naming the assistant and picking a theme, gated by a third Settings key (`onboarding_complete`) that `AppShell` checks on every route — skippable, and re-runnable via a link on the Settings page, not a one-time gate. `backend/app/domain`, `backend/app/services`, and `backend/app/repositories` each now hold one settings module (covering four keys, including `011`'s `wake_word_enabled`) and are otherwise still empty. No other feature work (STT/TTS/conversation, reminders, AI routing, media integrations, etc.) has started, and `frontend-mobile/`, `shared/`, `plugins/` (still no real plugin in it) remain placeholder directories only. Everything under `docs/` remains the design specification driving that future work.
