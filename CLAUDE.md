...
**Project Setup is complete** (see [docs/features/001_Project_Setup.md](docs/features/001_Project_Setup.md)): the monorepo scaffolding, backend skeleton, frontend skeleton, Docker, CI, and dev tooling all exist and are verified working. **Phase 0 is now fully closed** (001 through 009 all done — see [ROADMAP.md](ROADMAP.md)'s Phase 0 table). **Phase 1 (`010_Plugin_Framework`) is also done**: `backend/app/core/plugins.py` provides plugin discovery/registration, but no actual plugin exists yet. **Phase 2 has started with `011_Wake_Word` and `012_Speech_To_Text`, both done**: `backend/app/core/voice/` integrates OpenWakeWord and whisper.cpp as a single opt-in capability (`pip install '.[voice]'`) — the backend always boots safely without it, degrading to a documented push-to-talk fallback (`POST /api/v1/wake-word/trigger`, which now also drives STT); `wake_word_events` and `transcription_events` are the handoff contracts `013_TTS`/`015_Intent_Router` (neither started) will subscribe to, with `012` built entirely as a subscriber to `011`'s bus — no changes to `011`'s own code were needed. **`007_Dashboard` is done too**: the home route (`/`) is a real widget-hosting screen (greeting, clock, the existing health check, placeholder tiles for unbuilt features) — see `frontend-dashboard/src/components/dashboard/` and `frontend-dashboard/src/routes/DashboardPage.tsx`. **`008_Settings` is done as well**: `app_name`/`default_theme` are now DB-backed and writable at runtime (`GET`/`PATCH /api/v1/settings`, a Settings page in the dashboard) — this is the app's first real persistence, `backend/app/db/models.py`'s `SettingModel` plus `backend/alembic/`, introduced ahead of the originally-planned Phase 4 (see `BACKLOG.md`). **`009_Assistant_Onboarding` closes out Phase 0**: a first-run wizard (`frontend-dashboard/src/routes/OnboardingPage.tsx`, route `/onboarding`) walks a new user through naming the assistant and picking a theme, gated by a third Settings key (`onboarding_complete`) that `AppShell` checks on every route — skippable, and re-runnable via a link on the Settings page, not a one-time gate. `backend/app/domain`, `backend/app/services`, and `backend/app/repositories` each now hold one settings module (covering five keys, including `011`'s `wake_word_enabled` and `012`'s `stt_enabled`) and are otherwise still empty. No other feature work (TTS/conversation, intent routing, reminders, AI routing, media integrations, etc.) has started, and `frontend-mobile/`, `shared/`, `plugins/` (still no real plugin in it) remain placeholder directories only. Everything under `docs/` remains the design specification driving that future work.
...
- React Dashboard (web client) — **implemented** (`frontend-dashboard/`), including a Dashboard, a Settings page, and a first-run Onboarding wizard
